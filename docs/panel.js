...
const ImpactPanel = (() => {
  /* 评估核心与全部判定阈值已抽入 assess-core.js（前端与校准脚本共用的唯一尺子）。
     estGaleRadius 权威表仍在 data.js，此处注入。本文件只剩：取数、状态、渲染。 */
  AssessCore.configure({ estGaleRadius: TyphoonData.estGaleRadius });
  const { SOIL_DROP } = AssessCore;
  const haversine = AssessCore.haversine, ptime = AssessCore.ptime, warnRadius = AssessCore.warnRadius;
  const localImpactTier = AssessCore.localImpactTier;
  ...
  function assess(s) {
    const key = `${P.loc.lat},${P.loc.lng}`;
    const anteRec = P.antecedent[key];
    return AssessCore.assess({
      loc: P.loc,
      storm: s,
      fdata: P.forecast[key],
      soilW: (anteRec && typeof anteRec === "object") ? anteRec.w : 0,
      obs: nearestObs(P.loc.lat, P.loc.lng),
      nowT: Date.now(),
    });
  }
  ...
  async function loadAntecedent() {
    ...
    try {
      ...
      // Pa 公式在 core（soilFromDaily），与校准脚本同一把尺子
      P.antecedent[key] = AssessCore.soilFromDaily(series.slice(0, -1));
      renderResult();
  ...
  // 几何/时间/半径工具已迁入 assess-core.js（顶部已建 haversine/ptime/warnRadius 别名）
