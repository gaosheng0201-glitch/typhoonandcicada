...
判定核心是 **`docs/assess-core.js`**（前端与校准脚本共用的纯函数，「现在」永远是 nowT 参数）。
改它请走完整流程——阈值一变，同一场台风对同一座城市的结论就会变，必须可追溯、可回归：

1. **改前**先跑基线：`node scripts/sanity_check.mjs <tfid> --sweep`（多城市加 `--cities`，
   历史台风加 `--at 2026-07-04T12`）。判断标准是**整条时间线连贯**，不是单个时刻对不对。
2. 改 `assess-core.js`（不要在别处复制第二份口径——本项目已因「两套尺子」踩坑多次）。
3. **改后**重跑同一组命令对比；解除态相关的加 `--audit` 逐刻重演。
4. 同步文档：**METHODOLOGY.md**（口径全貌）与 **CHANGELOG.md**（为什么改 + 实测证据）。

每场台风停编 72h 后，CI（storm-audit）会自动全审计并把报告发布到
[`docs/data/audits/`](docs/data/audits/)——历史场次的判定表现全部公开可查，
每份报告附一键复现命令。发现某场审计结论与真实灾情不符，欢迎开 Issue（这类反馈
修出过本项目最重要的几个判定改进）。
